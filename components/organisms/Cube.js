        ...
        (upSideDown, start, stop, moving, startTouch, stopTouch, movingTouch, rotateX, rotateY, activeMove, wallSize) => (
          <ThemeProvider theme={{ wallSize }}>
            <StyledCube ref={cubeRef}>
              ...
              >
                <StyledWallLeft>
                  <ProjectOnCubeBox title={data?.[1]?.title} images={data?.[1]?.images} upSideDown={upSideDown} />
                </StyledWallLeft>
                <StyledWallRight>
                  <ProjectOnCubeBox title={data?.[0]?.title} images={data?.[0]?.images} upSideDown={upSideDown} />
                </StyledWallRight>
                <StyledWallTop>
                  <ProjectOnCubeBox title={data?.[2]?.title} images={data?.[2]?.images} />
                </StyledWallTop>
                <StyledWallBackRight>
                  <ProjectOnCubeBox title={data?.[3]?.title} images={data?.[3]?.images} upSideDown={upSideDown} />
                </StyledWallBackRight>
                <StyledWallBackLeft>
                  <ProjectOnCubeBox title={data?.[4]?.title} images={data?.[4]?.images} upSideDown={upSideDown} />
                </StyledWallBackLeft>
                <StyledWallBottom>
                  <ProjectOnCubeBox title={data?.[5]?.title} images={data?.[5]?.images} />
                </StyledWallBottom>
              </StyledCybeContainer>
            </StyledCube>
          </ThemeProvider>
